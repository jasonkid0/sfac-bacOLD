...
while($rows = mysqli_fetch_array ($l)){
  if($rows['curri'] == "New Curri"){ 
    $q = mysqli_query($db,"SELECT *, CONCAT(tbl_students.lastname, ', ', tbl_students.firstname, ' ', tbl_students.middlename)  as fullname,tbl_subjects_new.subj_id FROM tbl_enrolled_subjects
...
while($row = mysqli_fetch_array ($q)){
  $id=$row['enrolled_subj_id']; ?>
    <tr>
                          <td><?php echo'<input  type="checkbox" name="check[]" value="'.$id.'">'.'&nbsp;'.$row['subj_code']; ?></td>
...
<?php }
                  ...
                  <?php $p = mysqli_query($db, "SELECT * FROM tbl_students WHERE stud_id = '".$_GET['stud_id']."'");while($row = mysqli_fetch_array ($p)){ 
              if ($row['curri'] == 'Old Curri') {
                echo'
                  <a href="../enroll/select_subj2.php?stud_id='.$_GET['stud_id'].'&course='.$row['course_id'].'" class="btn btn-primary">
                    <i class="material-icons">add</i> Choose Subject
                  </a>';
              }else{
                echo'
                  <a href="../enroll/select_subj.php?stud_id='.$_GET['stud_id'].'&course='.$row['course_id'].'" class="btn btn-primary">
                    <i class="material-icons">add</i> Choose Subject
                  </a>';
                } ?>
                  <button type="submit" name="delete" class="btn btn-danger"><i class="material-icons">delete</i>Drop Selected</button>
                </div>
                  </form>
