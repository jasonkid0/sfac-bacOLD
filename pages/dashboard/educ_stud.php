            ...
            <div class="col-md-12">
              <?php 
                    ...
                    $eced = mysqli_fetch_array(mysqli_query($db,"SELECT COUNT(*) as total FROM `tbl_schoolyears` LEFT JOIN tbl_students ON tbl_students.stud_id = tbl_schoolyears.stud_id
                                  LEFT JOIN tbl_genders ON tbl_genders.gender_id = tbl_students.gender_id
                                  LEFT JOIN tbl_courses ON tbl_courses.course_id = tbl_schoolyears.course_id
                                  LEFT JOIN tbl_year_levels ON tbl_year_levels.year_id = tbl_schoolyears.year_id WHERE tbl_schoolyears.course_id = '17' AND ay_id = '$_SESSION[active_acad]' AND sem_id = '$_SESSION[active_sem]' AND remark = 'Approved'")) or die(mysql_error());
                    $eced1 = mysqli_fetch_array(mysqli_query($db,"SELECT COUNT(*) as total FROM `tbl_schoolyears` LEFT JOIN tbl_students ON tbl_students.stud_id = tbl_schoolyears.stud_id
                                  LEFT JOIN tbl_genders ON tbl_genders.gender_id = tbl_students.gender_id
                                  LEFT JOIN tbl_courses ON tbl_courses.course_id = tbl_schoolyears.course_id
                                  LEFT JOIN tbl_year_levels ON tbl_year_levels.year_id = tbl_schoolyears.year_id WHERE ay_id = '$_SESSION[active_acad]' AND sem_id = '$_SESSION[active_sem]' AND tbl_schoolyears.course_id = '17'  AND remark = 'Approved' AND status='Old'")) or die(mysql_error());
                    $eced2 = mysqli_fetch_array(mysqli_query($db,"SELECT COUNT(*) as total FROM `tbl_schoolyears` LEFT JOIN tbl_students ON tbl_students.stud_id = tbl_schoolyears.stud_id
                                  LEFT JOIN tbl_genders ON tbl_genders.gender_id = tbl_students.gender_id
                                  LEFT JOIN tbl_courses ON tbl_courses.course_id = tbl_schoolyears.course_id
                                  LEFT JOIN tbl_year_levels ON tbl_year_levels.year_id = tbl_schoolyears.year_id WHERE ay_id = '$_SESSION[active_acad]' AND sem_id = '$_SESSION[active_sem]' AND tbl_schoolyears.course_id = '17' AND remark = 'Approved' AND status='New'")) or die(mysql_error());
                    ...
                    ?>
                        ...
                        <!---   <a href="borrowed_book.php"><button class="btn btn-primary"><i class="fa fa-info"></i> All Records (<?php /// echo $count['total']; ?>)</button></a> -->
                            <button class="btn btn-success"><i class="fa fa-info"></i> BECED (<?php echo $eced['total']; ?>)<br>OLD (<?php echo $eced1['total']; ?>) , NEW (<?php echo $eced2['total']; ?>)</button>
